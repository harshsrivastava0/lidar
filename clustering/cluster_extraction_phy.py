...
def main(msg):
    ...
    for i in range(len(cloud.points)):
        if(labels[i]!=-1):
            point = cloud.points[i].tolist()

            #for implementing check
            if(labels[i] not in dict_cluster):
                dict_cluster[labels[i]] = []
                cluster_distance[labels[i]] = 0
            cluster_distance[labels[i]]+=(point[0]**2 + point[1]**2 + point[2]**2)**0.5
            cluster_distance[labels[i]]/=2
            dict_cluster[labels[i]].append(point)
    ...
    for i in dict_cluster:
        expectedpoints = 0.125*(1/(cluster_distance[i]**2)) * heightcone * widthcone * (1/math.tan(verticalres/2))* (1/math.tan(horizontalres/2))
        if(len(dict_cluster[i])<=expectedpoints+threshold and len(dict_cluster[i])>=expectedpoints-threshold):
            for j in dict_cluster[i]:
                temp = arr()
                temp.data = j
                temp.data.append(float(i))
                points_with_clusters.data.append(temp)
